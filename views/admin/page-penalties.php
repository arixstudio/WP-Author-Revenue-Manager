...
if (current_user_can( 'administrator'))
    arm_render_floating_button( array('data-bs-toggle'=>"modal", 'data-bs-target'=>"#arm-create-adjustment", 'title'=>__('Submit Penalty', 'rxarm')), '#');
...
// Do action
isset($_GET['action']) && $_GET['action'] == 'repeal' ?  arm_repeal_penalty_form() : '';

// Append url params
$params = array_merge( $params, $url_params );

// Get penalties
$penalties = arm_get_adjustments( $params );

// Get penalty records
$records = $penalties['records'];
...
$table_rows = array();
if (is_array($records))
    foreach ($records as $penalty) 
    {
        // Actions
        $actions = '<a href="'.add_query_arg( 'id', $penalty->id ).'">'.__('View','rxarm').'</a> ';
        if ($penalty->is_accounted != '1' && current_user_can( 'administrator' ))
          $actions .= '<a href="'.add_query_arg( array('action'=>'repeal','id'=> $penalty->id, '_wpnonce'=> wp_create_nonce( 'arm-repeal-penalty' )) ).'" class="needs-confirmation ms-2">'.__('Repeal','rxarm').'</a>';

        $table_rows[] = array(
            arm_get_user_fullname($penalty->author_id),
            arm_format_price( $penalty->amount , arm_get_author_locale($penalty->author_id) ),
            $penalty->reason,
            ($penalty->is_auto == '1' ? __( 'Auto generated', 'rxarm') : __( 'Custom', 'rxarm')),
            arm_handle_datetime($penalty->created_at, false),
            ($penalty->is_accounted == '1' ? __( 'Accounted', 'rxarm') : __( 'Not accounted', 'rxarm')),
            $actions,
        );
    }
...
// Submit penalty
function arm_submit_penalty_form()
{
  $result = arm_submit_penalty();
...
// Repeal penalty
function arm_repeal_penalty_form()
{
  $result = arm_repeal_penalty();
...
<div class="wrap arm-font <?php echo is_rtl() ? 'rtl' : '' ?>">  
  ...
  <?php isset($_POST['submit']) ?  arm_submit_penalty_form() : '' ?>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title"><?php _e('Submit Penalty', 'rxarm') ?></h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <form id="arm-penalty-form" class="arm-form" method="POST" action="">
            <div class="modal-body">
              ...
              <?php wp_nonce_field('arm-nonce-penalty', 'nonce', false) ?>
    ...
    <div id="arm-repeal-adjustment" class="modal fade" tabindex="-1" aria-labelledby="armModal" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title"><?php _e('Repeal penalty', 'rxarm') ?></h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
